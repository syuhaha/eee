...
<body>
  <div class="wrap">
    <div class="card">
      ...
      <form method="POST" action="{{ route('fyjt.verify') }}" id="fyjtStep1" novalidate>
        @csrf

        <div>
          <label for="token">aaa（）</label>
          <input id="token" name="token" type="text" maxlength="64" pattern="[A-Za-z0-9]{1,64}" inputmode="latin" autocomplete="off" value="{{ old('token') }}" required placeholder="请输入 Token">
        ...
        <div>
          ...
          <div class="captcha">
            ...
            <img id="captchaImg" src="{{ route('fyjt.captcha') }}?t={{ time() }}" alt="captcha" title="点击刷新" style="height:46px;border-radius:8px;border:1px solid var(--border);cursor:pointer">
  ...
  <script>
    (function(){
      ...
      function doRefresh(){ var now=Date.now(); if(now-lastAt<COOLDOWN_MS) return; lastAt=now; img.style.opacity='0.25'; img.style.transform='scale(.98)'; setTimeout(function(){ img.src='{{ route('fyjt.captcha') }}?t='+now; img.onload=function(){ img.style.opacity='1'; img.style.transform='scale(1)'; }; },160); setCooldown(COOLDOWN_MS); }
