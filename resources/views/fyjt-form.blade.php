...
<body>
  <div class="wrap">
    <div class="card">
      ...
      <form method="POST" action="{{ route('fyjt.submit') }}" id="fyjtForm" novalidate>
        @csrf

        <div class="field">
          <label for="token">aaa（）</label>
          <input id="token" name="token" type="text" maxlength="64" pattern="[A-Za-z0-9]{1,64}" inputmode="latin" autocomplete="off" value="{{ old('token') }}" required placeholder="请输入 Token（半角英数字）">
